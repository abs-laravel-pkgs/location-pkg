...
class CityController extends Controller {
		...
		return Datatables::of($states)
			...
			->addColumn('action', function ($state) {
				...
				return '
					...
					onclick="angular.element(this).scope().deleteCity(' . $state->id . ')" dusk = "delete-btn" title="Delete">
	...
	public function getCityFormData($id = NULL) {
		if (!$id) {
			$state = new City;
			$address = new Address;
			$action = 'Add';
		} else {
			$state = City::withTrashed()->find($id);
	...
	public function saveCity(Request $request) {
		...
		try {
			$error_messages = [
				...
				'code.unique' => 'City Code is already taken',
			...
			if (!$request->id) {
				...
				$state = City::withTrashed()->find($request->id);
	...
	public function deleteCity($id) {
		$delete_status = City::withTrashed()->where('id', $id)->forceDelete();
		if ($delete_status) {
